function [c, mag, ball1] = exampleHelperTurtleBotFindBlueBall(img, params, isHardware)
    ...
    if isempty(s)
        ...
        % Threshold so we don't chase images over the horizon on real
        % hardware
        if isHardware && ((c(2) < heightThresh) || (mag < imSize(1)/24))
            c = [];
            mag = [];
        end
    end
